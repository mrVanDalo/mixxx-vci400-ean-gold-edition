...
/*
Owen todo:
* add no-handler versions of 
* play / pause / etc so we can put out the lights.
*/
...
VestaxVCI400.enableMasterVu = true;
...
/**
 * Depending on what mode has been selected, 
 * the bottom row of buttons perform different functions.
 */
// not mapped !
...
//VestaxVCI400.Deck.prototype.setButtonMode = function(value) {
    ...
    //case VestaxVCI400.ModeEnum.ROLL:
        ...
        //break;
    //case VestaxVCI400.ModeEnum.SAMPLER:
        ////jthis.onSampler1DurationChanged(engine.getValue("[Sampler1]", "track_samples"), this.group);
...
/**
 * the Loop Move Wheel (move the loop or beatjump 
 * First version
 */
VestaxVCI400.loopMoveWheel = function (channel, control, value, status, group) {
    try{
        var jogValue = value - 0x40; // -64 to +63, - = CCW, + = CW
        ...
        if ( engine.getValue( group, "loop_enabled" ) == 1 ) {
            ...
            if(jogValue > 0) {
                ...
                engine.setValue(group, "beatjump_1_backward", 0);
            } else {
                engine.setValue(group, "beatjump_1_forward", 1);
                engine.setValue(group, "beatjump_1_forward", 0);
            }
        }
...
/** 
 * change the length of an active loop
 */
VestaxVCI400.loopLengthWheel = function (channel, control, value, status, group) {
    try{
        var jogValue = value - 0x40; // -64 to +63, - = CCW, + = CW
...
// can be written more save
VestaxVCI400.isPressed = function ( value ) {
    if ( value == 0x7F ){
        return true;
    } else { 
        return false;
...
/**
 * play in preview when pressed. but also stop playing
 */
VestaxVCI400.browserKnob = function (channel, control, value, status, group) {
    try{
        if ( VestaxVCI400.isPressed( value ) ) {
            if ( engine.getValue( group, "play" ) == 0) {
                // preview is not playing
                engine.setValue( group, "LoadSelectedTrackAndPlay", value);
            } else {
                // stop playing
 ...
 * play in preview when pressed. but also stop playing
 */
VestaxVCI400.browserWheel = function (channel, control, value, status, group) {
    try{
        if ( engine.getValue( "[PreviewDeck1]", "play" ) == 0 ) {
            // if preview is not playing
            // scrol the playlist
            if ( value == 0x01 ) {
            ...
            // if preview deck is playing
            // scrol in the preview deck
            if ( value == 0x01 ) {
                engine.setValue( "[PreviewDeck1]" , "beatjump_16_forward", 1);
            } else {
                engine.setValue( "[PreviewDeck1]" , "beatjump_16_backward", 1);
            }
...
/**
 * load a track ( only if track is not playing right now )
 */
...
VestaxVCI400.pitchKnob = function (channel, control, value, status, group) {
    try{
        ...
        if (VestaxVCI400.shiftActive) {
            ...
            if(jogValue > 0) {
                  engine.setValue(group, "pitch_down_small", 1);
                  engine.setValue(group, "pitch_down_small", 0);
            } else {
                  engine.setValue(group, "pitch_up_small", 1);
                  engine.setValue(group, "pitch_up_small", 0);
            }
        }
    }
    catch(ex) {
